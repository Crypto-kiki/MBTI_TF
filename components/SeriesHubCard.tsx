...
export function SeriesHubCard({ locale, series }: SeriesHubCardProps) {
  ...
  return (
    <article className={`glass-panel overflow-hidden rounded-[2rem] border border-white/10 p-5 shadow-soft sm:p-6 ${series.surfaceClass}`}>
      <div className="flex h-full flex-col gap-5">
        ...
        <div className="space-y-3">
          <p className="text-xs uppercase tracking-[0.2em] text-white/44">{series.content.eyebrow}</p>
          <h2 className="text-3xl font-semibold text-white sm:text-[2.15rem]">{series.content.title}</h2>
          <p className="text-base font-medium leading-7 text-white/86">{series.content.summaryLine}</p>
        ...
        <div className="grid gap-3 lg:grid-cols-2">
          <div className="surface-panel-muted px-4 py-4">
            ...
            <p className="mt-3 text-sm leading-7 text-white/76 sm:text-base">{series.content.topicSummary}</p>
          </div>
          <div className="surface-panel-muted px-4 py-4">
            <div className="flex items-center gap-2 text-white/74">
